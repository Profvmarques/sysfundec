...
function Processo($Processo) {
    ...
    switch ($Processo) {
        ...
        case 'editar':
            ...
            $periodoletivo->consultar("select *,date_format(data_inicio,'%d/%m/%Y') as dtini, date_format(data_fim,'%d/%m/%Y') as dtfim from periodoletivo order by siglacurso,siglaperiodo");
            $linha2=$periodoletivo->Linha;
            $rs2=$periodoletivo->Result;
            ...
            if ($_POST['ok'] == 'true') {
                try {
                    ...
                    $periodoletivo->consultar("BEGIN");
                    $semestre=substr($_POST['siglaperiodo'], -1); 
                    $periodoletivo->Alterar($_GET['id'],utf8_decode($_POST['siglacurso']),$semestre, utf8_decode($_POST['siglaperiodo']), $util->formatoDataYMD($_POST['data_inicio']), $util->formatoDataYMD($_POST['data_fim']));
                    $periodoletivo->consultar("COMMIT");
